 ...
 * @target MV MZ
 * 
 * @plugindesc [1.2.3] Custom Character Poses
...
function CpCacheManager() {
    throw new Error('This is a static class');
}

CpCacheManager.initialize = function() {
    this._cachedFiles = {};
    this._partyFiles = [];
    this._eventIndex = [];
    this._discardQueue = [];
...
CpCacheManager.initialize();

CpCacheManager.getCachedFile = function(file){
    return this._cachedFiles[file];
};

CpCacheManager.cacheFile = function(file, obj){
    if(this._discardQueue.contains(file)){
        this._discardQueue.splice(this._discardQueue.indexOf(file), 1);
    }
    if(this._cachedFiles[file] === undefined){
        this._cachedFiles[file] = ImageManager.loadCharacter(file);
        if(['Game_Player', 'Game_Follower'].contains(obj.constructor.name))
            this.registerPartyFiles(file);
        if(['Game_Event'].contains(obj.constructor.name))
            this.registerEventIndex(file);
    }
};

CpCacheManager.registerPartyFiles = function(file){
    this._partyFiles.push(file);
};

CpCacheManager.registerEventIndex = function(file){
    this._eventIndex.push(file);
};

CpCacheManager.prepareDiscardQueue = function(){
    for(var i=0; i<this._eventIndex.length; i++){
        if(!(this._partyFiles.contains(this._eventIndex[i]))){
            this._discardQueue.push(this._eventIndex[i]);
        }
    }
};

CpCacheManager.clearDiscardQueue = function(){
    for(var i=0; i<this._discardQueue.length; i++){
        delete this._cachedFiles[this._discardQueue[i]];
        this._eventIndex.splice(this._eventIndex.indexOf(this._discardQueue[i]), 1);
    }
    this._discardQueue = [];
};
...
Game_CharacterBase.prototype.setImage = function(characterName, characterIndex){
    if(characterName === this.characterName() && characterIndex === this.characterIndex()) return;
...
Game_CharacterBase.prototype.charImgExists = function(file){
    if(file === "") return true;
    var fs = require('fs');
    var filePath = 'img/characters/' + file + ".png";
    ...
    if(this._cpPoseLibrary !== undefined && Object.keys(this._cpPoseLibrary).length > 0){
        ...
        for(var i=0; i<keys.length; i++){
            ...
            if(pose['Spritesheet'] !== undefined && pose['Spritesheet'].length > 0 && this.charImgExists(pose['Spritesheet'])){
                CpCacheManager.cacheFile(pose['Spritesheet'], this);
            }
            if(pose['Blink'] !== undefined && pose['Blink'].length > 0 && this.charImgExists(pose['Blink'])){
                CpCacheManager.cacheFile(pose['Blink'], this);
            }
        }
    }
...
Game_CharacterBase.prototype.getCpBitmapCache = function(charFile){
    return CpCacheManager.getCachedFile(charFile);
};
...
Game_CharacterBase.prototype.startBlink = function(){
    ...
    this.setImage(pose['Blink'], pose['Blink Index']);
...
Game_Player.prototype.refresh = function(){
    ...
    if(this.isPoseAvailable(actor._cpCurrentPose)){
...
Game_Player.prototype.initializePoses = function(){
    var actor = $gameParty.leader();
    if(TAA.cp.Parameters.Actor[actor.actorId()] !== null){
...
TAA.cp.alias.Sprite_Character.setCharacterBitmap = Sprite_Character.prototype.setCharacterBitmap;
Sprite_Character.prototype.setCharacterBitmap = function(){
    var bitmapCache = this._character.getCpBitmapCache(this._characterName);
    if(bitmapCache !== undefined){
        this.bitmap = bitmapCache;
        this._isBigCharacter = ImageManager.isBigCharacter(this._characterName);
    }
    else
        TAA.cp.alias.Sprite_Character.setCharacterBitmap.call(this);
};
...
Game_Map.prototype.setupEvents = function() {
    CpCacheManager.prepareDiscardQueue();
    TAA.cp.alias.Game_Map.setupEvents.call(this);
    CpCacheManager.clearDiscardQueue();
};