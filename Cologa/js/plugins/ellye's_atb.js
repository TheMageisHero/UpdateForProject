 ...
 * 
 *
 * @plugindesc  V.2015-11-14-2144. A simple Active Time Battle system.
 * <Ellye ATB>
...
(function() {
    ...
    if (display_gauges === 1)
    {
        ...
        Window_Base.prototype.drawActorATB = function(actor, x, y, width) {
            ...
            if (actor.target_cast_atb !== 0)
            {
                ...
                this.drawActorATBGauge(x, y, width, actor.castRate(), color1, color2, actor.castName(), x, y, gauge_text_width);
            }
            //ATB:
            else
            {
                this.drawActorATBGauge(x, y, width, actor.atbRate(), atb_gauge_color1, atb_gauge_color2, gauge_name, x, y, gauge_text_width);
            }
        };
    ...
    if (display_enemy_atb_gauges === 1 || display_enemy_cast_gauges === 1)
    {
        ...
        Sprite_Enemy.prototype.updateBitmap = function() {
            ...
            if (this._gaugeWindow === null && typeof this.bitmap !== 'undefined' && this.bitmap !== null && this.bitmap.height !== 0)
            {
                ...
                this._gaugeWindow.initialize(-this.bitmap.width / 2, y, this.bitmap.width + 36, this._gaugeWindow.lineHeight() + 2 * this._gaugeWindow.standardPadding() + 26);
            ...
            if (this._gaugeWindow !== null && this._battler !== null && BattleManager._phase === 'atb')
            {
                ...
                else if (this._battler.target_cast_atb !== 0 && display_enemy_cast_gauges === 1)
                {
                    ...
                    this._gaugeWindow.drawGauge(0, 0, this.bitmap.width, this._battler.castRate(), color1, color2);
                    if (enemy_show_cast_name === 1)
                    {
                        this._gaugeWindow.drawText(this._battler.castName(), 0, 0, this.bitmap.width);
                    }
                    else if (enemy_show_cast_name === 2)
                    {
                        this._gaugeWindow.drawText(cast_gauge_name, 0, 0, this.bitmap.width);
                    }
                }
                else if (display_enemy_atb_gauges === 1)
                {
                    this._gaugeWindow.drawGauge(0, 0, this.bitmap.width, this._battler.atbRate(), enemy_gauge_color1, enemy_gauge_color2);
                    if (enemy_show_name === 1)
                    {
                        this._gaugeWindow.drawText(this._battler.originalName(), 0, 0, this.bitmap.width);
                    }
                    else if (enemy_show_name === 2)
                    {
                        this._gaugeWindow.drawText(gauge_name, 0, 0, this.bitmap.width);
                    }
                }
            }
        };
    ...
    //And one for calculating Cast Rate for th gauge:
    ...
    CalculateATBRate = function(agi) {
        return (base_atb_increase + agi * (agi_weight / 100));
    };
    ...
    Game_Battler.prototype.calculateATBRate = function() {
        return CalculateATBRate(this.agi);
    };
        ...
        do
        {
            ...
            this.allBattleMembers().forEach(function(battler)
            {
                ...
                else if (battler.target_cast_atb > 0)
                {
                    ...
                    {
                        battler.current_cast_atb += battler._cast_rate;
    ...
    BattleManager.predictTurnOrder = function(battlersToApplyModifiersTo, modifiersValues, battlerCasting, castingTime)
    {
        ...
        while (predictedTurnOrder.length < numberOfTurns)
        {
            for (index = 0; index < arrayOfATBValues.length; index++)
            {
                ...
                {
                    arrayOfCastValues[index] += battler._cast_rate;
                    if (arrayOfCastValues[index] >= target_cast)
                    {
    ...
    Game_Battler.prototype.calculateCastRate = function()
    {
        ...
        if (typeof eval(param) !== 'undefined' && !isNaN(eval(param)))
        {
            ...
            this._cast_rate = CalculateATBRate(cs);
        }
        else
        {
            this._cast_rate = CalculateATBRate(0);
        }
    };
})();