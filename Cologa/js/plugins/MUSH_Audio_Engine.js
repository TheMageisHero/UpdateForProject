...
/*:
* @target MZ
* @plugindesc [v.1.03] Implement the Mush Audio Engine features.
* @author MushroomCake28
...
* @arg FadeIn
* @text Fade In
* @desc Set a fade in duration in number of frames. Leave it at 0 if you don't want any fade in.
* @type number
* @min 0
* @default 0
*
...
Imported.mushFeatures['MUSH_Audio_Engine'] = 1.03;
...
AudioManager._meLastBgmVolume = null;
AudioManager._meLastBgsVolume = null;
...
AudioManager.playMe = function(me) {
	if (me.name) {
        if (this._meBuffer == null) {
        	this._meLastBgmVolume = this.bgmVolume;
        	this._meLastBgsVolume = this.bgsVolume;
        	this.bgmVolume = 0;
        	this.bgsVolume = 0;
        }
    }
...
AudioManager.stopMe = function() {
    if (this._meBuffer) {
        ...
        this.bgmVolume = this._meLastBgmVolume;
        this.bgsVolume = this._meLastBgsVolume;
        this._meLastBgmVolume = null;
        this._meLastBgsVolume = null;
    }
};
	...
	if (type == 'bgs') {
		...
		if (mus.reduceMain) {
			if (AudioManager._currentBgm) {
				...
				var mainBgm = {name: $dataMap.bgm.name, pan: $dataMap.bgm.pan, pitch: $dataMap.bgm.pitch, volume: mainMax};
