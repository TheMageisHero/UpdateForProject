...
var Vitsuno = Vitsuno || {};
Vitsuno.AbilityNet = {};
Vitsuno.AbilityNet.version = 1.10;
 ...
 * (0:Always show -1:Never show)
 * @default 0
 * 
 * @param Level UP Point
 * @desc Number of points acquired at level up.(a = actor)
 ...
 * 
 * Required plug-in:
 * To use this plugin, Vitsuno_SpritePerch.js is required.
 * Please place it so that it is below on the plugin management screen.
 *
 * Required image: Please put the following file in img / system.
 * PerchCursor.png # Cursor image for image command (square square pattern side by side)
...
 *
 * First, we set the abilities (armor) ID that becomes the starting point in the memoranda of the actor.
 * Number of points required to acquire the ability · Set the next ability ID.
 * We will set it for the next abilities the same way.
 *
 * Notes of actors and occupations:
 * <abilityNetBases: ID 1, ID 2, ID 3> # Starting abilities ID to add
 *
 * Protector notes:
 * <abilityNetPoint: num> # Number of points required for learning
 * <abilityNetNexts: ID1, ID2, ID3> # next ability ID
 * <abilityNetBaseName: name> # List display name for origin abilities
 *
 * <abilityNetOverwritten: ID> # Ability ID to overwrite this ability
 *
 * <abilityNetPosition: x, y> # Set the abilities' coordinates (optional)
 *
 * Memo of enemy character:
 * <dropAbilityNetPoint: num> # Number of points to be acquired when knocked down
 *
 * Occupation notes:
 * <abilityNetworkId: ID> # occupation ID sharing abilities
 *
 * Plugin command:
 * AbilityNetwork GainActorPoint Actor ID Rising # Increase points
 * AbilityNetwork LearnActorAbility Actor ID Ability ID # Master Ability
 * AbilityNetwork ClearActorAbility Actor ID # Clear learning state
 *
 * AbilityNetwork GoToScene Actor Party Order # Move to Ability Screen (Order from 0)
 *
...
 * Ability point If the remaining amount becomes negative, the acquired ability is reset.
 * Ability points do not decrease when level is down.
 * The ability rise of Ability overwritten ability · The effect of the feature is ignored.
 * Moving the screen of the plug-in command will not move if there are no characters in the order.
 *
 * About ability selection:
 * When selecting an ability, the destination of the cursor at the time of pressing up, down, left, and right depending on the distance between abilities is determined,
 * Depending on the position, there may be abilities that can not be selected, so please test it thoroughly.
 * If you can not select it, adjust the number by increasing or decreasing the number of abilities, or changing the derivation.
 *    
 */
...
DataManager.extractMetadata = function(data) {
    ...
    if (data.atypeId !== undefined) {
        ...
        // 上書きするアビリティID
        if (typeof data.meta.abilityNetOverwritten === 'string') {
            data.abilityNetOverwritten = data.meta.abilityNetOverwritten.split(",").map(Number);
        }
        // アビリティ座標
...
Game_Actor.prototype.abilityNetItems = function() {
    ...
    return items.filter(function(obj, i, list) {
        return this.includesAbilityNetItems(obj, list);
    }, this);
};

// ● アビリティアイテム配列に含めるか判定
Game_Actor.prototype.includesAbilityNetItems = function(item, items) {
    var overwrites = item.abilityNetOverwritten || [];
    for (var i = 0; i < overwrites.length; i++) {
        if (items.contains($dataArmors[overwrites[i]])) {
            return false;
        }
    }
    return true;
    ...
    if (command === 'AbilityNetwork') {
        switch (args[0]) {
            case 'GainActorPoint':
                ...
                {
                    console.log("Didn't Give Points");
                }
                break;
