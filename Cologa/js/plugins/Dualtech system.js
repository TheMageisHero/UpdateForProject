...
//=============================================================================
 /*:
 * @plugindesc Adds functionality for dualtech skills.
 *
 ...
 * <Dual:ID1,Skill1,ID2,Skill2,HitRate>
 *
 ...
 * ID2 and Skill2 likewise refer to the second character.
 *
 ...
 * differently in the future.) You may define the following values for Hit Rate:
 *
 * user - Uses the user's hit rate.
 * first - Uses the first actor's (ID1's) hit rate.
 * second - Uses the second actor's (ID2's) hit rate.
 * higher - Uses the hit rate of the actor whose hit rate is higher.
 * lower - Uses the hit rate of the actor whose hit rate is lower.
 * sum - Adds the hit rates of the two actors.
 *
 ...
 * which do not change based on which character selected the skill.
 *
 ...
 *
 * Teaching a dualtech skill to someone other than those defined by ID1 and ID2 will
 * only allow its use if all THREE characters are in the party - a triple tech, if you
 * will. However, it will not be tied to a skill of the person who learns it.
 *
 * If you define the dualtech as a component for itself, then you drop the dependency
 * on another skill and instead use the costs and restrictions defined to the
 * dualtech. One or both characters may be set this way. It is not recommended to set
 * a skill as a component to itself for a triple tech unless it is hidden from the two
 * actors defined in the skill (because, remember, they will see the skill as a dualtech,
 * not a triple tech.)
 *
 ...
 *
 * It is not required that both members of the dualtech learn the dualtech skill -
 * if Joe and Bob have a dualtech, you can teach it only to Joe and it will still work.
 * However, Bob will not be able to initiate the dualtech. Bob must still learn the
 * associated skill either way, though.
 *
...
Jay.Dualtechs.dualSkillData = function(dualData) {
	...
	returnParams.push(Number(dualParams[0]));

	if(isNaN(dualParams[1])) {
		returnParams.push($dataSkills.filter(function(skill) { return skill !== null && skill.name === dualParams[1]; 

})[0].id);
	}
	else {
		returnParams.push(Number(dualParams[1]));
	}

	returnParams.push(Number(dualParams[2]));

	if(isNaN(dualParams[3])) {
		returnParams.push($dataSkills.filter(function(skill) { return skill !== null && skill.name === dualParams[3]; 

})[0].id);
	}
	else {
		returnParams.push(Number(dualParams[3]));
	}

	if(dualParams.length > 4) {
		returnParams.push(dualParams[4]);
	}
...
Window_SkillList.prototype.includes = function(item) {
	...
	var id1 = dualParams[0]
	var skill1 = dualParams[1];
	var id2 = dualParams[2];
	var skill2 = dualParams[3];

	var actorList = $gameParty.members();

	if (actorList.indexOf($gameActors.actor(id1)) === -1 || actorList.indexOf($gameActors.actor(id2)) === -1) {
		return false;
	}

	if(!$gameActors.actor(id1).skills().contains($dataSkills[skill1])) {
		return false;
	}

	if(!$gameActors.actor(id2).skills().contains($dataSkills[skill2])) {
		return false;
	}
...
Window_SkillList.prototype.drawSkillCost = function(skill, x, y, width) {
	...
	var endWidth = width;
	var thirdActor = this._actor !== $gameActors.actor(dualParams[0]) && 
		this._actor !== $gameActors.actor(dualParams[2]);

	
	if (!Jay.Param.CycleActors || this._cyclePhase === 1 || (!thirdActor && (this._cyclePhase === 3 ||
		this._cyclePhase === 5)) || (thirdActor && this._cyclePhase === 4)) {
			endWidth = this.drawComponentSkillCost($gameActors.actor(dualParams[2]), 
				$dataSkills[dualParams[3]], x, y, width);
	}

	if (!Jay.Param.CycleActors || this._cyclePhase === undefined || this._cyclePhase === 0 || 
		(!thirdActor && (this._cyclePhase === 2 || this._cyclePhase === 4)) || 
		(thirdActor && this._cyclePhase === 3)) {
			endWidth = this.drawComponentSkillCost($gameActors.actor(dualParams[0]), 
				$dataSkills[dualParams[1]], x, y, endWidth);
	}

	if (thirdActor && (!Jay.Param.CycleActors || this._cyclePhase === 2 || this._cyclePhase === 5)) {
...
Game_BattlerBase.prototype.meetsSkillConditions = function(skill) {
	if(skill.meta.Dual) {	
		...
		var dualParams = Jay.Dualtechs.dualSkillData(skill.meta.Dual);
		if(!Jay.Dualtechs.meetsSkillConditions.call($gameActors.actor(dualParams[0]), $dataSkills[dualParams[1]]) || 
			!Jay.Dualtechs.meetsSkillConditions.call($gameActors.actor(dualParams[2]), $dataSkills[dualParams
...
[3]])) {
		...
		if(this == $gameActors.actor(dualParams[0]) || this == $gameActors.actor(dualParams[2])) {
...
Game_BattlerBase.prototype.paySkillCost = function(skill) {
	if(skill.meta.Dual) {	
		var dualParams = Jay.Dualtechs.dualSkillData(skill.meta.Dual);
		Jay.Dualtechs.paySkillCost.call($gameActors.actor(dualParams[0]), $dataSkills[dualParams[1]]);
		Jay.Dualtechs.paySkillCost.call($gameActors.actor(dualParams[2]), $dataSkills[dualParams[3]]);

		// No extra cost for using the skill for the two main actors!
		if(this.actor() === $gameActors.actor(dualParams[0]).actor() || 
			this.actor() === $gameActors.actor(dualParams[2]).actor()) {
			return;
		}
	}
...
Game_Action.prototype.evalDamageFormula = function(target) {
	...
	try {
		var item = this.item();
		var dualParams = Jay.Dualtechs.dualSkillData(item.meta.Dual);
		var c = $gameActors.actor(dualParams[0]);
		var d = $gameActors.actor(dualParams[2]);
		var a = this.subject();
		var b = target;
    ...
    if (this.isPhysical()) {
        ...
        if (this.item().meta.Dual) {
            ...
            if (dualParams.length > 4) {
                ...
                case "first":
                case "actor1":
                    hitRate = $gameActors.actor(dualParams[0]).hit;
                    break;
                case "second":
                case "actor2":
                    hitRate = $gameActors.actor(dualParams[2]).hit;
                    break;
                case "higher":
                case "highest":
                    var hitRate1 = $gameActors.actor(dualParams[0]).hit;
                    var hitRate2 = $gameActors.actor(dualParams[2]).hit;
                    hitRate = Math.max(hitRate1, hitRate2, hitRate);
                    break;
                case "lower":
                case "lowest":
                    var hitRate1 = $gameActors.actor(dualParams[0]).hit;
                    var hitRate2 = $gameActors.actor(dualParams[2]).hit;
                    hitRate = Math.min(hitRate1, hitRate2, hitRate);
                    break;
                case "sum":
                    var hitRate1 = $gameActors.actor(dualParams[0]).hit;
                    var hitRate2 = $gameActors.actor(dualParams[2]).hit;
                    hitRate = hitRate1 + hitRate2;
                    break;
                }
            }
        }
